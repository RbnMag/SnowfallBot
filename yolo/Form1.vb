...
Public Class Form1
    ...
    Private Sub sentapi(ByVal message As String, Optional ByVal user As String = "rbnmag")
        ...
        If message.StartsWith("#trace") Then
            Dim list() As String = Split(message)
            Dim request As HttpWebRequest = HttpWebRequest.Create("http://api.predator.wtf/geoip/?arguments=" & list(1))
        ...
        If message.StartsWith("#resolve") Then
            Dim list() As String = Split(message)
            Dim request As HttpWebRequest = HttpWebRequest.Create("http://apis.skypebot.ga/apis/resolve.php?u==" & list(1))
            Dim response As HttpWebResponse = request.GetResponse
            Dim source As String = New IO.StreamReader(response.GetResponseStream).ReadToEnd
    ...
    Private Sub recievedapi(ByVal message As String, ByVal user As String, ByVal chat As Chat)
        ...
        If InStr(My.Settings.admins, user) Then
            ...
            If message.StartsWith("#showprems") Then
                Dim prems As String = ""
                prems = prems & My.Settings.ddos & My.Settings.autobuyprems & " | Temporary Premiums: " & My.Settings.tempprem
        ...
        If message.StartsWith("#cfresolve") Then
            Dim x As New Threading.Thread(DirectCast(Sub() cfresolve(message, user, chat), Threading.ThreadStart))
            x.Start()
            Exit Sub
        End If
        ...
        If message = "ey b0ss" Then
            chat.SendMessage("gibe the poosi pls.")
            Exit Sub
        End If
        ...
        If message.StartsWith("#shownigger") Then
            chat.SendMessage("https://img.4plebs.org/boards/pol/image/1407/08/1407081155171.png")
            Exit Sub
        End If
        ...
        If message.StartsWith("#resolve") Then
            ...
            If InStr(My.Settings.blacklistedskypenames, list(1)) Then
                ...
                Dim request2 As HttpWebRequest = HttpWebRequest.Create("http://apis.skypebot.ga/apis/resolve.php?u=" & list(1))
                Dim response2 As HttpWebResponse = request2.GetResponse
                Dim source2 As String = New IO.StreamReader(response2.GetResponseStream).ReadToEnd
            Else
                ...
                Next
                If checksum <= 5 Then
                    If InStr(message.ToLower, "rbnmag") Or InStr(message.ToLower, "elite.reality") Or InStr(message.ToLower, "robbedshark") Or InStr(message.ToLower, "outgrows") Or InStr(message.ToLower, "sex.gov") Or InStr(message.ToLower, "onlinedude1999") Or InStr(message.ToLower, "bad-killer12345") Or InStr(message.ToLower, "jessepinkman.") Or InStr(message.ToLower, "dannyboi2544") Or InStr(message.ToLower, "enxhell3") Then
        ...
        If message.StartsWith("#skypedb") Then
            Dim list() As String = Split(message)
            If InStr(message.ToLower, "outgrows") Then
                chat.SendMessage("Nice try, yet again.")
                chat.SendMessage("If you want to resolve any of the admins, then don't use the bot.")
            Else
                Dim request As HttpWebRequest = HttpWebRequest.Create("http://api.predator.wtf/lookup/?arguments=" & list(1))
        ...
        If message.StartsWith("#dns") Then
            On Error Resume Next
            Dim x As New Threading.Thread(DirectCast(Sub() dnslookup(message, user, chat), Threading.ThreadStart))
            x.Start()
            Exit Sub
        End If
        ...
        If message.StartsWith("#e2s") Then
            On Error Resume Next
            Dim x As New Threading.Thread(DirectCast(Sub() e2s(message, user, chat), Threading.ThreadStart))
            x.Start()
            Exit Sub
        End If
        ...
        If message.StartsWith("#joke") Then
            On Error Resume Next
            Dim x As New Threading.Thread(DirectCast(Sub() joke(message, user, chat), Threading.ThreadStart))
            x.Start()
            Exit Sub
        End If

        If message.StartsWith("#mccheck") Then
            On Error Resume Next
            Dim x As New Threading.Thread(DirectCast(Sub() MCchecker(message, user, chat), Threading.ThreadStart))
            x.Start()
            Exit Sub
        End If

        If message.StartsWith("#useless") Then
            On Error Resume Next
            Dim sentChat As ChatMessage = chat.SendMessage("Creating link...")
            Dim request As HttpWebRequest
            Dim list() As String = Split(message)
            request = HttpWebRequest.Create("http://api.predator.wtf/google/?arguments=" & list(1))
            Dim response As HttpWebResponse = request.GetResponse
            Dim source As String = New IO.StreamReader(response.GetResponseStream).ReadToEnd
            sentChat.Body = source
        End If

        If message.StartsWith("#lyle") Then
            chat.SendMessage("Why are you doing a command about a stoner? You stupid.")
            Exit Sub
        End If


        If message.StartsWith("#portcheck") Then
            On Error Resume Next
            Dim x As New Threading.Thread(DirectCast(Sub() portCheck(message, user, chat), Threading.ThreadStart))
            x.Start()
            Exit Sub
        End If

        If message.StartsWith("#lemny") Then
            On Error Resume Next
            chat.SendMessage("http://soundcloud.com/toxic_overdrive")
            Exit Sub
        End If
        ...
        If message.StartsWith("#passgen") Then
            On Error Resume Next
            Dim x As New Threading.Thread(DirectCast(Sub() passwordgen(message, user, chat), Threading.ThreadStart))
            x.Start()
            Exit Sub
        End If

        If message.StartsWith("#asaprocky") Then
            chat.SendMessage("ere")
            Exit Sub
        End If

        If message.StartsWith("#isup") Then
            On Error Resume Next
            Dim x As New Threading.Thread(DirectCast(Sub() isup(message, user, chat), Threading.ThreadStart))
            x.Start()
            Exit Sub
        End If
    ...
    Private Sub portCheck(ByVal message As String, ByVal user As String, ByVal chat As Chat)
        On Error Resume Next
        Dim list() As String = Split(message)
        Dim sentChat As ChatMessage = chat.SendMessage("Checking port...")
        Dim request As HttpWebRequest
        list(1) = list(1).ToLower
        request = HttpWebRequest.Create("http://api.predator.wtf/pcheck/?arguments=" & list(1) & "&port=" & list(2))
        Dim response As HttpWebResponse = request.GetResponse
        Dim source As String = New IO.StreamReader(response.GetResponseStream).ReadToEnd
        sentChat.Body = source
    End Sub

    Private Sub passwordgen(ByVal message As String, ByVal user As String, ByVal chat As Chat)
        On Error Resume Next
        Dim list() As String = Split(message)
        Dim sentChat As ChatMessage = chat.SendMessage("Creating Password...")
        Dim request As HttpWebRequest
        list(1) = list(1).ToLower
        request = HttpWebRequest.Create("http://api.predator.wtf/passgen/?arguments=" & list(1))
        Dim response As HttpWebResponse = request.GetResponse
        Dim source As String = New IO.StreamReader(response.GetResponseStream).ReadToEnd
        sentChat.Body = source
    End Sub

    Private Sub dnslookup(ByVal message As String, ByVal user As String, ByVal Chat As Chat)
        On Error Resume Next
        Dim list() As String = Split(message)
        Dim sentChat As ChatMessage = Chat.SendMessage("Looking up DNS records...")
        Dim request As HttpWebRequest
        list(1) = list(1).ToLower
        request = HttpWebRequest.Create("http://api.predator.wtf/dns/?arguments=" & list(1))
    ...
    Private Sub MCchecker(ByVal message As String, ByVal user As String, ByVal chat As Chat)
        On Error Resume Next
        Dim list() As String = Split(message)
        Dim sentChat As ChatMessage = chat.SendMessage("Checking Minecraft Name...")
        Dim request As HttpWebRequest
        list(1) = list(1).ToLower
        request = HttpWebRequest.Create("http://api.predator.wtf/minecraft/?arguments=" & list(1))
        Dim response As HttpWebResponse = request.GetResponse
        Dim source As String = New IO.StreamReader(response.GetResponseStream).ReadToEnd
        sentChat.Body = source
    End Sub

    Private Sub isup(ByVal message As String, ByVal user As String, ByVal chat As Chat)
        On Error Resume Next

        Dim list() As String = Split(message)
        Dim request As HttpWebRequest
        Dim sentchat As ChatMessage = chat.SendMessage("Checking...")
        list(1) = list(1).ToLower
        request = HttpWebRequest.Create("http://api.predator.wtf/upordown/?arguments=" & list(1))
        Dim response As HttpWebResponse = request.GetResponse
        Dim source As String = New IO.StreamReader(response.GetResponseStream).ReadToEnd
        sentchat.Body = source
    End Sub
    ...
    Private Sub resolve(ByVal message As String, ByVal user As String, ByVal chat As Chat)
        ...
        flash.Start()
        If PurpleKushRadioButton1.Checked Then
            request = HttpWebRequest.Create("http://apis.skypebot.ga/apis/resolve.php?u=" & list(1))
        ElseIf PurpleKushRadioButton2.Checked Then
            request = HttpWebRequest.Create("http://vbooter.org/api/skype.php?username=" & list(1))
        Else
            request = Nothing
        End If
        Dim response As HttpWebResponse = request.GetResponse
    ...
    Private Sub cfresolve(ByVal message As String, ByVal user As String, ByVal chat As Chat)
        On Error Resume Next
        ...
        Dim flash As New Threading.Thread(DirectCast(Sub() flasheffect(sentChat), Threading.ThreadStart))
        flash.Start()
        request = HttpWebRequest.Create("http://api.predator.wtf/cfresolve/?arguments=" & list(1))
        Dim response As HttpWebResponse = request.GetResponse
        Dim source As String = New IO.StreamReader(response.GetResponseStream).ReadToEnd
        Dim source2 = ""
        flash.Abort()
    ...
    Private Sub joke(ByVal message As String, ByVal user As String, ByVal chat As Chat)
        On Error Resume Next
        Dim list() As String = Split(message)
        Dim request As HttpWebRequest
        Dim sentChat As ChatMessage = chat.SendMessage("M")
        list(1) = list(1).ToLower
        Dim flash As New Threading.Thread(DirectCast(Sub() jokeeffect(sentChat), Threading.ThreadStart))
        flash.Start()
        request = HttpWebRequest.Create("http://api.predator.wtf/joke/?arguments=nerdy")
        Dim response As HttpWebResponse = request.GetResponse
        Dim source As String = New IO.StreamReader(response.GetResponseStream).ReadToEnd
        Dim source2 = ""
        flash.Abort()
    ...
    Private Sub e2s(ByVal message As String, ByVal user As String, ByVal chat As Chat)
        On Error Resume Next
        ...
        Dim flash As New Threading.Thread(DirectCast(Sub() flasheffect(sentChat), Threading.ThreadStart))
        flash.Start()
        request = HttpWebRequest.Create("http://api.predator.wtf/e2skype/?arguments=" & list(1))
        Dim response As HttpWebResponse = request.GetResponse
        Dim source As String = New IO.StreamReader(response.GetResponseStream).ReadToEnd
        Dim source2 = ""
        flash.Abort()
...
lol:
    ...
    Private Sub jokeeffect(ByVal chat As ChatMessage)
        On Error Resume Next
        Threading.Thread.Sleep(100)
        chat.Body = "M"
        Threading.Thread.Sleep(100)
        ...
        chat.Body = "Making jo"
        Threading.Thread.Sleep(100)
        chat.Body = "Making jok"

lol:
        Threading.Thread.Sleep(100)
        chat.Body = "Making joke"
        Threading.Thread.Sleep(100)
        chat.Body = "Making joke."
        Threading.Thread.Sleep(100)
        chat.Body = "Making joke.."
    ...
    Private Sub ip2skype(ByVal ip As String, ByVal chat As Chat)
        Dim request As HttpWebRequest = HttpWebRequest.Create("http://api.predator.wtf/lookup/?arguments=" & ip)
        Dim response As HttpWebResponse = request.GetResponse
        Dim source As String = New IO.StreamReader(response.GetResponseStream).ReadToEnd
...
again:
    ...
    Private Sub PurpleKushRadioButton3_CheckedChanged(sender As Object)
